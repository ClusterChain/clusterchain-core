package org.clusterchain.core;


import static org.apache.commons.lang3.ArrayUtils.isEmpty;
import static org.clusterchain.util.ByteUtil.EMPTY_BYTE_ARRAY;
import static org.clusterchain.util.ByteUtil.ZERO_BYTE_ARRAY;
...
import org.clusterchain.crypto.ECKey;
import org.clusterchain.crypto.ECKey.ECDSASignature;
import org.clusterchain.crypto.ECKey.MissingPrivateKeyException;
import org.clusterchain.crypto.HashUtil;
import org.clusterchain.experiment.ExperimentConfig;
import org.clusterchain.util.ByteUtil;
import org.clusterchain.util.RLP;
import org.clusterchain.util.RLPElement;
import org.clusterchain.util.RLPItem;
import org.clusterchain.util.RLPList;
...
public class Transaction implements Serializable {
  ...
  protected byte[] sendAddressFromSignature;
  ...
  public synchronized byte[] getSenderFromSignature(){
    try {
      if(sendAddressFromSignature == null && getSignature() != null)
        sendAddressFromSignature =ECKey.signatureToAddress(getRawHash(), getSignature());
      return sendAddressFromSignature;
  ...
  public boolean verifySignature(){
    if(!ExperimentConfig.verifySig) {
      sendAddressFromSignature=sendAddress;
      return true;
    }
    byte[] sigSender= getSenderFromSignature() ;
    //   System.err.println("sender="+Hex.toHexString(sendAddress)+",  senderFromSig="+Hex.toHexString(sigSender));
    if(sigSender!=null && sendAddress!=null && Arrays.equals(sigSender,sendAddress))
      return true;
    return false;
  }
...
//  }
  ...
  public synchronized void rlpParse() {
    if (parsed) return;
    //   System.out.println("TTTTTTTTTTT:"+rlpEncoded.length);
    try {
      ...
      this.sendAddress=  transaction.get(6).getRLPData();
  ...
  public byte[] getEncodedRaw() {
    ...
    byte[] sendAddr= RLP.encodeElement(this.sendAddress);
  ...
  public byte[] getEncoded() {
    ...
    this.rlpEncoded = RLP.encodeList(nonce, gasPrice, gasLimit,
        receiveAddress, value, data, sendAddr,v, r, s);
  ...
  public static List<Transaction> readObjs(){
    List<Transaction> ret=new ArrayList<>();
    String serialFile="E:\\intellijProjects\\ethereumj-1.6.3\\directBlocks\\transaction.txt";
    try{
